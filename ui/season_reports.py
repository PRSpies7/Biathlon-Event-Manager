...
def render(db_path, snapshot):
    ...
    st.divider()
    st.subheader("Qualified schools", help="Ps identifies primary schools; Hs identifies high schools. Schools rank by their selected team's season total. Primary: U9, U11, U13, U15 plus two extras. High: U15, U17, U19 plus three extras. Missing mandatory groups leave only their own places empty; up to two primary or three high-school extras still count. Extras cannot replace missing mandatory places. Qualified replacements take priority; boys and girls share places. At most one special-needs athlete can fill an extra place, capped at 2,000 points. Each selected athlete needs four distinct completed meets including an interprovincial or GN Championship, and a best score of at least 1,890. The athlete sheet shows every event score and missing requirements. School names and qualification cells are highlighted.")
